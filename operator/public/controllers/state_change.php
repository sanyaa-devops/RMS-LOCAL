...
$id_check = "SELECT * FROM `flight_rotation` WHERE `slot_start_time` LIKE '$slot' AND customer_id = '$cid' AND `status` != 'ongoing' and `status` != 'complete' and `tunnel_hold` != '1' ORDER BY order_by LIMIT 0, 1;";
...
function findHoldTime($slot, $lastHoldTime){
    ...
    $currentSlot = "SELECT * FROM `flight_rotation` WHERE `slot_start_time` LIKE '$slot' AND start_time = '$time' and `tunnel_hold` != '1' ORDER BY order_by ASC;";
...
if ($resultHold) {
    if (in_array($status, $status_array)) {
        ...
        if ($status === 'schedule') {
            ...
            $currentSlot = "SELECT * FROM `flight_rotation` WHERE `slot_start_time` LIKE '$slot' and `tunnel_hold` != '1' ORDER BY order_by ASC;";
            $currentSlot = outputs($currentSlot);
    ...
    END WHERE `id` IN (" . $uids . ") AND `status` != 'hold' and `tunnel_hold` != '1';";
            inputs($updateQuery);
            $query = "UPDATE `flight_rotation` SET `status` = 'hold', start_time = '" . $time . "' WHERE `id` = '$resultHold->id' and `tunnel_hold` != '1'  and status not in ('ongoing','complete')";
            $result = inputs($query);
            updateNext();
        } else {
            ...
            // if ($currentSlotIsRunning && $currentSlotIsRunning > 0) {
                $lastCard = "SELECT * FROM `flight_rotation` WHERE `slot_start_time` LIKE '$slot' and `tunnel_hold` != '1' ORDER BY order_by DESC LIMIT 0, 1;";
                $lastCard = outputs($lastCard);
                $lastCard = $lastCard->fetch_object();

                $holdRow = "SELECT * FROM `flight_rotation` WHERE `slot_start_time` LIKE '$slot' AND customer_id = '$cid' AND `status` = 'hold' and `tunnel_hold` != '1'";
