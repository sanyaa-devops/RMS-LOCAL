...
$query1 = "SELECT id,duration FROM flight_rotation WHERE slot_start_time = '$slot_start_time' AND (status = 'next' OR status = 'schedule') ORDER BY order_by";
...
while ($row = mysqli_fetch_array($result1)) {
    ...
    $qry = "UPDATE flight_rotation SET start_time = '$start_time', end_time = '$end_time' WHERE id = $id";
    inputs($qry);

    $start_time = $end_time; //setting last end time as start time for next iteration
    $slot_time = new DateTime($end_time);
}
removeNext();
