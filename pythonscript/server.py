...
def fetch_data():
    query = """
        SELECT SQL_NO_CACHE
            ...
            GROUP_CONCAT(FR.tunnel_hold) AS g_th,
...
def stream(ip, port, password, path, type):
    username = "gopro"
    ...
    if type == "both":
        try:
            url = f"https://{ip}/gopro/camera/stream/start?port={port}"
            response = requests.get(
                url,
                timeout=10,
                headers={"Authorization": f"Basic {token}"},
...
def start_streaming_for_cameras():
    ...
    for idx, camera_ip in enumerate(camera_ips):
        ...
        if flag:
            ...
            successful_cameras.append((ip_address,camera_name,camera_ip[6],camera_ip[7]))
            stream(camera_ip[3],camera_ip[4],camera_ip[6],camera_ip[7],"both")
        else:
            update_camera_status(camera_ip[0],output,"1")
            successful_cameras.append((ip_address,camera_name,camera_ip[6],camera_ip[7]))
...
async def check_ip_reachability(successful_camera_ips):
    ...
    while not should_exit:
        for entry in successful_camera_ips:
            ip, camera_name = entry[0], entry[1]
            ...
            status_message = {
                ...
                "output": output,  # Optionally include the output of the ping
                "timestamp": time.time()
            }
...
async def main():
    ...
    print("WebSocket server started on ws://10.3.2.100:55777")
